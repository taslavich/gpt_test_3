	...
	for _, campaign := range snapshot.Campaigns {
		...
		for _, imp := range req.GetImp() {
			...
			if preflightOK {
				...
				logf("[ADV][RTB_PREFLIGHT_REJECT] request_id=%q campaign_id=%q imp_id=%q reason=%q", strings.TrimSpace(req.GetId()), campaign.ID, imp.GetId(), rejectReason)
			}
		}
		if len(eligible) > 0 {
...
func (s *AuctionService) evaluateRTBCandidate(ctx context.Context, campaign *Campaign, bid *ortb.Bid, imp *ortb.Imp, now time.Time, requestedFormat, exactSegmentHash, segmentHash string) (candidate, bool, error) {
	...
	if normalizeTypeModel(campaign.TypeModel) == TypeModelSimple {
		simplePricing := s.resolveSimplePricing(ctx, campaign, exactSegmentHash, segmentHash, rawPrice, pricing.MinMargin, pricing.MapSource, true, now)
...
func (s *AuctionService) writeRTBResponseStats(_ context.Context, impUUID map[string]string, stats map[string]map[string]string, logf debugLogFunc) {
	...
	for impID, rawUUID := range impUUID {
		...
		logf("[ADV][RTB_STATS_WRITE_OK] imp_id=%q uuid=%q items=%v", impID, uuid, items)
	}
}
