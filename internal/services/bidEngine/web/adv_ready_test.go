...
func TestRTBWinnerIsNotReportedAsADVWinner(t *testing.T) {
	requestID, impID, uuid := "request-rtb", "imp-1", "uuid-1"
	advPrice := float32(0.50)
	...
	request := &bidEngineGrpc.BidEngineRequest_V2_5{
		...
		WinnerBasePrices: map[string]float64{impID: 0.75},
		SspDomain:        "ssp.example",
		Format:           constants.POP,
		Logged:           false,
		Rekl:             false,
	...
	if response.GetCode() != 200 || response.GetBidResponse() == nil {
		t.Fatalf("unexpected RTB response: code=%d response=%v", response.GetCode(), response.GetBidResponse())
	}
	if response.GetRekl() {
		t.Fatal("RTB winner must not be reported as all-ADV/rekl")
	}
	if _, exists := response.GetWinnerUserIds()[impID]; exists {
		t.Fatal("RTB winner must not expose the losing ADV winner user")
	}
	if got := response.GetImpIdUuidClone()[impID]; got != uuid {
		t.Fatalf("RTB uuid clone=%q want %q", got, uuid)
	}
}
