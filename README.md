...
### Как зафиксировать внешний IP MetalLB

Скрипт `deploy.sh` автоматически пытается определить диапазон адресов MetalLB. Если у узла есть внешнее значение `ExternalIP`, оно будет использовано как **единственный** адрес пулла. При необходимости можно указать IP вручную, экспортировав переменную окружения перед запуском:
...
Адрес, указанный в `METALLB_IP_ADDRESS`, будет назначен `gateway-service` и `ingress-nginx`. Это избавляет от неожиданного выбора произвольного IP из диапазона.
