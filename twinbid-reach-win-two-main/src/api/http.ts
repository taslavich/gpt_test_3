...
export async function http<T>(path: string, opts: RequestOptions = {}): Promise<T> {
  ...
  if (!res.ok) {
    ...
    // For expired/invalid session, show a friendly message instead of the raw token error.
    if (res.status === 401) {
      const lang = (typeof navigator !== "undefined" && navigator.language || "").toLowerCase().startsWith("ru")
        ? "ru"
        : "en";
      const message =
        lang === "ru"
      ...
      throw new ApiError(401, message, err?.code || "SESSION_EXPIRED", err?.fields);
    }
    throw new ApiError(
      res.status,
