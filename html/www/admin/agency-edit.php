...
function buildAgencyForm($aAgency)
{
    ...
    if (empty($aAgency['agencyid'])) {
        $form->addElement('header', 'header_manager', $GLOBALS['strUserDetails']);
        $form->addElement('text', 'manager_username', $GLOBALS['strUsername']);
        $form->addElement('text', 'manager_email', $GLOBALS['strEMail']);
        $form->addElement('password', 'manager_password', $GLOBALS['strPassword']);
        $form->addElement('password', 'manager_password_confirm', $GLOBALS['strPasswordRepeat']);
    }
    ...
    if (empty($aAgency['agencyid'])) {
        $usernameRequiredMsg = $translation->translate($GLOBALS['strXRequiredField'], [$GLOBALS['strUsername']]);
        $form->addRule('manager_username', $usernameRequiredMsg, 'required');

        $managerEmailRequiredMsg = $translation->translate($GLOBALS['strXRequiredField'], [$GLOBALS['strEMail']]);
        $form->addRule('manager_email', $managerEmailRequiredMsg, 'required');
        $form->addRule('manager_email', $GLOBALS['strEmailField'], 'email');

        $passwordRequiredMsg = $translation->translate($GLOBALS['strXRequiredField'], [$GLOBALS['strPassword']]);
        $form->addRule('manager_password', $passwordRequiredMsg, 'required');
        $form->addRule(['manager_password_confirm', 'manager_password'], $GLOBALS['strNotSamePasswords'], 'compare');
    }
...
function processForm($aAgency, $form)
{
    ...
    if ($isNewAgency) {
        ...
        $oPlugin->validateUsersPassword($aFields['manager_password'], $aFields['manager_password_confirm']);

        $aValidationErrors = $oPlugin->getValidationErrors();
        if (!empty($aValidationErrors)) {
            foreach ($aValidationErrors as $error) {
                if ($error === $GLOBALS['strInvalidEmail']) {
                    $form->setElementError('manager_email', $error);
                } elseif (in_array($error, [$GLOBALS['strInvalidPassword'], $GLOBALS['strPasswordTooShort'], $GLOBALS['strNotSamePasswords']], true)) {
                    $form->setElementError('manager_password', $error);
                } else {
                    $form->setElementError('manager_username', $error);
                }
            }

            displayAgencyEditPage($aAgency, $form);
            return;
        }
    }
    ...
    if (empty($aFields['agencyid'])) {
        ...
        if ($agencyid) {
            $doAgency = OA_Dal::staticGetDO('agency', $agencyid);
            $accountId = (int) $doAgency->account_id;

            $oPlugin = OA_Auth::staticGetAuthPlugin();
            $userId = $oPlugin->saveUser(
                null,
                $aFields['manager_username'],
                $aFields['manager_password'],
                $agency['contact'],
                $aFields['manager_email'],
            ...
            if (!$userId) {
                $doAgency->delete();
                $form->setElementError('manager_username', $GLOBALS['strUserNotLinkedWithAccount']);
                displayAgencyEditPage($aAgency, $form);
                return;
            }
